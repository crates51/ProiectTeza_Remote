					    ...
					    <b-form-group id="input-group-10" label="Room :" label-for="input-10">
		        			...
		        			<b-form-invalid-feedback>
	        					...
	        					<span v-if="!$v.form.specificRoom.roomExist">
        						  This building has a total of {{roomsTotal}} rooms and starts from 1.
	        					</span>
...
export default {
  ...
  //       },
    ...
    validations: {
      	form: {
			...
			roomExist(value) {
		        if (value === '') return true
		        return new Promise((resolve, reject) => {
		            resolve(value <= this.roomsTotal && value > 0)
    ...
    created(){
    	axios.get('api/rooms')  
              .then(response => { 
    			this.roomsTotal=response.data.rooms.length;
      	});
    },
    mounted(){},
	methods:{
          ...
          const Toast_this_customer = Swal.mixin({
            ...
            Swal.fire({
              ...
              allowOutsideClick:false,
	            ...
	            if (result.value) {
	            	this.form.client_choosed=true;
          			this.form.last_name=clientFound.Last_Name;
          			this.form.first_name=clientFound.First_Name;
          			this.form.email=clientFound.Email;
